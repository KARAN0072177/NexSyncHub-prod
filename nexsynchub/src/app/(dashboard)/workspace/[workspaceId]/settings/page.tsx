...
export default function WorkspaceSettingsPage() {
  ...
  const [moderationModal, setModerationModal] = useState({ isOpen: false, title: "", message: "" });
  ...
  const handleSave = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        // 🔥 AI moderation blocked
        if (
          data.error?.toLowerCase().includes("guideline") || data.reason
        ) {
          setModerationModal({
            isOpen: true,
            title: "Action Blocked",
            message: data.reason || data.error || "This content violates our community guidelines."
          });
          return;
        }
        return showToast(data.error || "Failed to save changes", "error");
      }
      setSavedPulse(true);
  ...
  const uploadCroppedAvatar = async (croppedBlob: Blob) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        if (data.error?.toLowerCase().includes("guideline") || data.reason || data.error?.toLowerCase().includes("unsafe")) {
          setModerationModal({
            isOpen: true,
            title: "Avatar Blocked",
            message: data.reason || data.error || "This image violates our community guidelines due to unsafe or prohibited content."
          });
          return;
        }
        return showToast(data.error || "Failed to upload avatar", "error");
      }
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  background:
                    `linear-gradient(90deg, #FF0000, transparent)`,
              ...
              <div className="p-7">
                ...
                <div className="flex items-start justify-between mb-5">
                  ...
                  <button
                    ...
                    onClick={() => setModerationModal(prev => ({ ...prev, isOpen: false }))}
