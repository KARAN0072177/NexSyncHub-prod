  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        if (res.ok) {
          setEnabled(data.settings.allowRegistrations);
        }
      } catch (error) {
        console.error(error);
      } finally {
  ...
  const handleToggle = async () => {
    try {
      ...
      const res = await fetch("/api/admin/platform-settings", {
        ...
        body: JSON.stringify({ allowRegistrations: newValue }),
      });
      ...
      if (!res.ok) {
        ...
        showToast(
          newValue
            ? "Registrations have been enabled."
      ...
      >
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-start justify-between gap-8">
          <div className="flex items-start gap-5">
            ...
            >
              {enabled ? (
                <UserPlus size={24} style={{ color: T.emerald }} />
              ) : (
            ...
            <div>
              ...
              <p
                ...
                style={{ color: T.muted }}
              >
                Control whether new visitors can create accounts on your
                NexSyncHub instance. Disabling this locks the registration page
                globally, but existing users can still log in normally.
              </p>
          ...
          <button
            ...
            aria-label={
              enabled ? "Disable registrations" : "Enable registrations"
            }
          >
            <motion.div
        ...
        {/* Warning block when registrations disabled */}
        <AnimatePresence>
          {!enabled && (
            <motion.div
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  >
                    Registration is strictly disabled
                  </p>
                  <p
                    className="text-sm leading-relaxed"
                    style={{ color: "rgba(255,255,255,0.7)" }}
                  >
                    The signup page will now display a "Registrations Closed"
                    message. Ensure you re-enable this when you are ready to
