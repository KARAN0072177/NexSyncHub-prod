...
import { Building2, Loader2, ShieldAlert, CheckCircle2, XCircle, Lock } from "lucide-react";
  ...
  const handleToggle = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        showToast(newValue ? "Workspace creation is now allowed." : "Workspace creation is now disabled.", "success");
      }
    } catch (error) {
      console.error(error);
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-start justify-between gap-8">
          <div className="flex items-start gap-5">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-inner transition-colors duration-500"
              ...
              {enabled ? <Building2 size={24} style={{ color: T.emerald }} /> : <Lock size={24} style={{ color: T.rose }} />}
            </div>
            ...
            <div>
              ...
              <p className="text-sm leading-relaxed max-w-2xl" style={{ color: T.muted }}>
                Control whether users can create new workspaces on the platform. Disabling this prevents any new workspace from being created globally.
              </p>
          ...
          {!enabled && (
            <motion.div initial={{ opacity: 0, height: 0, marginTop: 0 }} animate={{ opacity: 1, height: "auto", marginTop: 32 }} exit={{ opacity: 0, height: 0, marginTop: 0 }} className="overflow-hidden">
              <div className="rounded-2xl p-5 flex items-start gap-4" style={{ background: T.roseLo, border: `1px solid ${T.roseMd}` }}>
                ...
                <div>
                  <p className="font-bold text-sm mb-1" style={{ color: "#fff", fontFamily: "'Sora',sans-serif" }}>Workspace creation is strictly disabled</p>
                  <p className="text-sm leading-relaxed" style={{ color: "rgba(255,255,255,0.7)" }}>
                    Users will no longer be able to create new workspaces until this setting is re-enabled. Existing workspaces will continue to function normally.
                  </p>
                </div>
              </div>
            </motion.div>
